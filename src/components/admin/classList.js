    ...
    const result = dateInfos.map((info, i) => {
      if (info.date) {
        const matched = classProfile.filter((profile) => {
          const profileDateString = profile.date.toLocaleDateString();
  ...
  const getMonths = () => {
    if (classProfile) {
      ...
      classProfile.forEach((profile) => {
        const month = profile.date.toDate().getMonth();
        const year = profile.date.toDate().getFullYear();
