...
const RescheduleSingle = ({
  ...
  const pendingDate = pendingClassInfo && pendingClassInfo.date;
  const rescheduleDate = rescheduleClassInfo && rescheduleClassInfo.date;
  const handleClick = (e) => {
    e.preventDefault();
  ...
  const reschedulePendingOutput = () => {
    ...
    const userIndex =
      pendingClassInfo && pendingClassInfo.pendingStudents.indexOf(uid) + 1;
    return (
