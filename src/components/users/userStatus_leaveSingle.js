    ...
    switch (status) {
      case 'rescheduled':
        let rescheduledData = () => {
          const rescheduleDate =
            rescheduleClassInfo && rescheduleClassInfo.date.toDate();
      ...
      case 'reschedulePending':
        let reschedulePendingData = () => {
          const pendingDate =
            pendingClassInfo && pendingClassInfo.date.toDate();
        ...
        return (
          <ul className="comfyList">
            <li>
              {`候補${reschedulePendingData().dateString}，順位：${
                reschedulePendingData().pendingIndex
              }`}
              <br />
            ...
            <li>
              ...
              <span className="action">
                <Link
                  to="/reschedule"
                  onClick={() => {
                    clearSuccessMessage('RESCHEDULE');
                  }}
                >
                  現在去安排
                </Link>
